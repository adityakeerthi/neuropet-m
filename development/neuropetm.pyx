...
cpdef setup_environment():
    # Folders to create
    cdef list folders, oslistfolders
    folders, oslistfolders = ["Client", "NPETM.Scans"], os.listdir()
    cdef dict config = {  }

    for folder in folders:
        if folder not in oslistfolders: os.makedirs(folder)

    with open("./Client/config.json", "w") as json_file:
        json.dump(config, json_file)
    
    instructions = open("./Client/instructions.txt", "w+")
    instructions.write("To set up the environment, please insert folders named with the type of the radioactive tracer. \nInside each respective folder, pleace include the OASIS-3 PET Scans and then set threshold values for each PET Scan in config.json. \n ")
    instructions.close()

    return True

cpdef edit_threshold():
    cdef double lf, hf
    cdef int distance
    cdef dict config

    with open("./Client/config.json") as json_file: config = json.load(json_file)

    title = input("\nPlease enter a distinct name for the produced PET Scan: ")
    config["title"] = title
    for tracer in os.listdir("./Client"):
        if "." in tracer: continue
        distance = 0
        print("\nPlease set the threshold values for the following tracer: " + tracer)
        while distance != 1:
            print("Remember the maximum distance between the lower and higher threshold is 1.0.")
            lf = float(input("The Lower Threshold: "))
            hf = float(input("The Higher Threshold: "))
            if abs(lf - hf) <= 1.0: distance = 1

        config[tracer] = { "lower_threshold": lf, "higher_threshold": hf }

    print("You have successfully edited the threshold values of the corresponding PET Scans :)")

    with open("./Client/config.json", "w") as json_file: json.dump(config, json_file)
...
cpdef produce_scans_OASIS3():
    ...
    for tracer in os.listdir("./Client"):
        ...
        for file_name in os.listdir("./Client/" + tracer + "/Scans"):
            os.remove("./Client/" + tracer + "/Scans/" + file_name)

        for file_name in os.listdir("./Client/" + tracer + "/Data"):
            os.remove("./Client/" + tracer + "/Data/" + file_name)
...
cpdef process_scan_path(scan_path):
    ...
    lower_threshold, higher_threshold = config[tracer]["lower_threshold"], config[tracer]["higher_threshold"]
...
cpdef produce_multimodal_scan():
    ...
    dataset = generate_pdset()
    c_discrete_map = dataset["~colour_discrete_map"]
    x, y, z, keys, tracers = dataset["~x"], dataset["~y"], dataset["~z"], dataset["~keys"], dataset["~tracer"]

    fig = px.scatter_3d(x=x, y=y, z=z, color=keys, color_discrete_map=c_discrete_map, opacity=0.5, template="plotly_dark", title=config["title"])

    fig.write_html("./NPETM.Scans/" + config["title"] + ".html")
